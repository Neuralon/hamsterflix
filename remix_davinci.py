...
# Voiceover (with the 3 characters we generated earlier)
vo_path = os.path.join(TRAILERS_DIR, f"../voiceovers/{UID}_final_vo.mp3")
vid_path = os.path.join(TRAILERS_DIR, f"temp_{UID}.mp4")
final_path = os.path.join(TRAILERS_DIR, f"trailer_{UID}.mp4")
bgm_path = os.path.join(TRAILERS_DIR, "real_davinci.mp3")
concat_txt = os.path.join(TRAILERS_DIR, f"veo_scenes_{UID}", "concat.txt")

print("Mixing Real Da Vinci Code Soundtrack (Hans Zimmer)...")
...
# BGM volume down to 0.45, VO volume up to 2.5. Skipping the first 45 seconds of the song to get straight to the epic choral buildup.
subprocess.run([
    "ffmpeg", "-y", "-i", vid_path, "-i", vo_path, "-stream_loop", "-1", "-ss", "00:00:45", "-i", bgm_path,
...
print("COMPLETED THE HAMSTER CODE MIX!")
