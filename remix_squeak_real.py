...
bgm_path = os.path.join(TRAILERS_DIR, "real_babe.mp3")
concat_txt = os.path.join(TRAILERS_DIR, f"veo_scenes_{UID}", "concat.txt")

print("Mixing REAL Babe Soundtrack...")
...
# Real music from Babe, dropping volume slightly and fading out at 70s.
subprocess.run([
    "ffmpeg", "-y", "-i", vid_path, "-i", vo_path, "-stream_loop", "-1", "-ss", "00:00:00", "-i", bgm_path,
    "-filter_complex", "[2:a]volume=0.6,afade=t=out:st=70:d=5[bg];[1:a]volume=2.5[vo];[vo][bg]amix=inputs=2:duration=longest:dropout_transition=2[a_out]",
...
print("COMPLETED SQUEAK REAL MIX!")
