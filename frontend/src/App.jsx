...
function Report() {
  ...
  const toggleApproval = async (uid, currentStatus) => {
    ...
    // Optimistic update
    setReportData(prev => prev.map(m => m.uid === uid ? { ...m, approved: newStatus } : m));
    try {
      await fetch(`/api/movies/${uid}/approve`, {
        method: 'POST',
      ...
      // Revert on error
      setReportData(prev => prev.map(m => m.uid === uid ? { ...m, approved: currentStatus } : m));
    }
  };

  const updateNotes = async (uid, notes) => {
    setReportData(prev => prev.map(m => m.uid === uid ? { ...m, notes } : m));
    try {
      await fetch(`/api/movies/${uid}/notes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const missingTrailers = reportData.filter(m => m.trailerStatus !== 'Ready (Veo)');

  return (
    <div className="min-h-screen bg-netflix-dark text-white pt-24 px-4 md:px-12 pb-12">
      <div className="max-w-6xl mx-auto space-y-8">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold mb-2">Completion Report</h1>
          <p className="text-gray-400">Total Movies: {reportData.length} | Missing Veo Trailers: {missingTrailers.length}</p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="bg-gray-800 text-gray-300">
                <tr>
                  <th className="p-4 border-b border-gray-700">Movie Title</th>
                  <th className="p-4 border-b border-gray-700">Type</th>
                  <th className="p-4 border-b border-gray-700">Script Status</th>
                  <th className="p-4 border-b border-gray-700">Trailer Status</th>
                  <th className="p-4 border-b border-gray-700">Music</th>
                  <th className="p-4 border-b border-gray-700">Voiceover</th>
                  <th className="p-4 border-b border-gray-700">Cast Images</th>
                  <th className="p-4 border-b border-gray-700">Approve</th>
                  <th className="p-4 border-b border-gray-700">Notes</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-800">
                {reportData.map((m, i) => (
                  <tr key={i} className={`hover:bg-gray-800/50 ${m.approved ? 'opacity-50' : ''}`}>
                    <td className="p-4 font-medium"><Link to={`/movie/${m.uid}`} className="text-white hover:underline">{m.title}</Link></td>
                    <td className="p-4 text-gray-400">{m.type}</td>
                    <td className="p-4">
                      {m.hasScript ? <span className="text-green-500">Ready</span> : <span className="text-red-500 font-bold">Missing</span>}
                    </td>
                    <td className="p-4">
                      {m.trailerStatus === 'Ready (Veo)' ? (
                        <span className="text-green-500">{m.trailerStatus} {m.trailerLen !== '-' && <span className="text-gray-500 text-xs ml-1">({m.trailerLen})</span>}</span>
                      ) : m.trailerStatus === 'Partial / Generic' ? (
                        <span className="text-yellow-500">{m.trailerStatus} {m.trailerLen !== '-' && <span className="text-gray-500 text-xs ml-1">({m.trailerLen})</span>}</span>
                      ) : (
                        <span className="text-red-500 font-bold">{m.trailerStatus}</span>
                      )}
                    </td>
                    <td className="p-4">
                      {m.musicStatus === 'Real Soundtrack' ? (
                        <span className="text-green-500">{m.musicStatus} {m.musicLen !== '-' && <span className="text-gray-500 text-xs ml-1">({m.musicLen})</span>}</span>
                      ) : m.musicStatus === 'Synth' ? (
                        <span className="text-yellow-500">{m.musicStatus} {m.musicLen !== '-' && <span className="text-gray-500 text-xs ml-1">({m.musicLen})</span>}</span>
                      ) : m.musicStatus === 'Placeholder' ? (
                        <span className="text-gray-400">{m.musicStatus}</span>
                      ) : (
                        <span className="text-red-500 font-bold">{m.musicStatus}</span>
                      )}
                    </td>
                    <td className="p-4">
                      {m.voStatus.startsWith('Ready') ? (
                        <span className="text-green-500">{m.voStatus} {m.voLen !== '-' && <span className="text-gray-500 text-xs ml-1">({m.voLen})</span>}</span>
                      ) : (
                        <span className="text-red-500 font-bold">{m.voStatus}</span>
                      )}
                    </td>
                    <td className="p-4 text-gray-300">{m.actorImagesCount} / 20</td>
                    <td className="p-4">
                      <input 
                        type="checkbox" 
                        checked={m.approved === 1}
                        onChange={() => toggleApproval(m.uid, m.approved)}
                        className="w-5 h-5 accent-netflix-red cursor-pointer rounded border-gray-700 bg-gray-800"
                      />
                    </td>
                    <td className="p-4">
                      <input
                        ...
                        onBlur={(e) => {
                          if (e.target.value !== m.notes) {
                            updateNotes(m.uid, e.target.value);
                          }
                        }}
                        placeholder="Add notes..."
                        className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1 text-sm text-white focus:outline-none focus:border-netflix-red placeholder-gray-500"
                      />
                    </td>
                  </tr>
                ))}
...
function ScriptsReview() {
  const [scripts, setScripts] = React.useState([]);

  React.useEffect(() => {
    window.scrollTo(0, 0);
    fetch('/trailer_scripts.json')
      .then(res => res.json())
      .then(data => setScripts(data))
      .catch(e => console.error("Could not load scripts", e));
  }, []);

  return (
    <div className="min-h-screen bg-netflix-black text-white pt-24 px-4 md:px-12 pb-20">
      <Navbar />
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center gap-4 mb-10">
          <FileText className="w-10 h-10 text-netflix-red" />
          <h1 className="text-4xl font-bold">Trailer Scripts Master List</h1>
        </div>
        <p className="text-gray-400 mb-8 text-lg">
          Review the generated trailer treatments for all {scripts.length} movies. The styles have been heavily varied across Voiceover, Dialogue, and pure Music/Action.
        </p>

        <div className="space-y-8">
          {scripts.map((script, idx) => (
            <div key={idx} className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden shadow-lg">
              <div className="bg-black/50 px-6 py-4 border-b border-gray-800 flex justify-between items-center">
                <h2 className="text-xl font-bold flex items-center gap-3">
                  <span className="bg-netflix-red text-white text-xs px-2 py-1 rounded">ID: {script.uid.substring(0, 6)}...</span>
                  {script.inspiration && <span className="text-yellow-400 text-sm italic font-normal">Inspired by: {script.inspiration}</span>}
                </h2>
                <span className="text-sm font-medium text-gray-400 border border-gray-700 px-3 py-1 rounded-full">
                  {script.trailer_style}
                </span>
              </div>
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <h3 className="text-sm text-gray-500 uppercase tracking-wider mb-2 font-bold">Audio & Tone Direction</h3>
                  <p className="text-gray-300 mb-6 bg-black/30 p-4 rounded-lg italic">
                    {script.audio_direction}
                  </p>
                  
                  <h3 className="text-sm text-gray-500 uppercase tracking-wider mb-2 font-bold">Script / On-Screen Text</h3>
                  <div className="text-gray-200 bg-gray-800/50 p-4 rounded-lg whitespace-pre-wrap font-mono text-sm leading-relaxed border-l-4 border-netflix-red">
                    {script.script_content || "[No dialogue/text generated]"}
                  </div>
                </div>
                
                <div>
                  <h3 className="text-sm text-gray-500 uppercase tracking-wider mb-2 font-bold">15-Scene Storyboard</h3>
                  <div className="space-y-2 h-[300px] overflow-y-auto pr-4 hide-scrollbar custom-scroll">
                    {script.scenes && script.scenes.map((scene, s_idx) => (
                      <div key={s_idx} className="flex gap-3 text-sm bg-black/20 p-3 rounded">
                        <span className="text-gray-600 font-bold font-mono">{String(s_idx + 1).padStart(2, '0')}</span>
          ...
          {scripts.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-gray-400">Generating master script file in the background...</p>
            </div>
          )}
        </div>
      </div>
    ...
    <Router>
      <div className="min-h-screen bg-netflix-black selection:bg-netflix-red selection:text-white">
        <Routes>
          ...
          <Route path="/scripts" element={<ScriptsReview />} />
