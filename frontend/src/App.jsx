...
const FEATURED_MOVIE = {
  ...
  heroImage: "/posters_real/poster_real_1.png"
};
...
function Hero({ featured }) {
  ...
  const movieImg = `${import.meta.env.VITE_MEDIA_BASE || ""}${heroMovie.heroImage}`;
  return (
    <div className="relative h-[80vh] w-full">
      <div className="absolute inset-0">
        <img src={movieImg} alt="Hero Background" className="w-full h-full object-cover" />
        ...
        .then(data => {
          if (data && data.length > 0) {
             ...
             filteredData.forEach((movie, i) => {
               ...
               const inspiration = (movie.inspiration || '').toLowerCase();
               const strictTags = genres + ' ' + inspiration;
               ...
               if ((strictTags.includes("family") || strictTags.includes("animation") || strictTags.includes("kids") || strictTags.includes("wonka") || strictTags.includes("zootopia") || strictTags.includes("pixar") || strictTags.includes("babe") || strictTags.includes("e.t.")) && !["horror", "thriller", "terror", "scary", "suspense", "saw ", "alien", "jaws", "revenant", "shawshank", "midnight express", "blade runner", "dune", "ocean", "maze runner", "fast & furious", "great escape", "doctor strange", "justice league", "crazy rich", "armageddon", "indiana jones", "mad max", "bond", "girl next door"].some(t => strictTags.includes(t))) {
               ...
               if (strictTags.includes("adventure") || strictTags.includes("epic") || strictTags.includes("indiana jones") || strictTags.includes("mystery") || strictTags.includes("documentary") || strictTags.includes("beyond") || strictTags.includes("whimsical") || strictTags.includes("enchanting") || strictTags.includes("sherlock") || strictTags.includes("sixth sense") || strictTags.includes("x-files") || strictTags.includes("jurassic") || strictTags.includes("mummy")) {
                 beyondTheTubes.push(movie);
               } 
               if (!strictTags.includes("comedy") && !strictTags.includes("horror") && !strictTags.includes("action") && !strictTags.includes("alien") && !strictTags.includes("saw") && !strictTags.includes("jaws") && !strictTags.includes("x-files") && !strictTags.includes("sixth sense") && !strictTags.includes("girl next door")) {
             ...
             if (filteredData.length > 0) {
               setFeatured({
                 ...
                 heroImage: filteredData[0].img || (filteredData[0].poster_filename ? '/posters_real/' + filteredData[0].poster_filename : '/posters_real/poster_real_1.png')
               });
             }
          }
        })
...
function MovieDetail() {
      ...
      const movieImg = details.img || (details.poster_filename ? '/posters_real/' + details.poster_filename : '/posters_real/poster_real_1.png');
      const absoluteMovieImg = `${import.meta.env.VITE_MEDIA_BASE || ""}${movieImg}`;
          ...
          {isPlaying ? (
            ...
            <>
              ...
              <div className="fixed inset-0 z-0">
                <img src={absoluteMovieImg} alt="background blur" className="w-full h-full object-cover opacity-20 blur-3xl scale-110" />
                ...
                <div className="flex flex-col md:flex-row gap-12">
                  ...
                  <div className="w-full md:w-1/3 shrink-0">
                    <div className="rounded-xl overflow-hidden shadow-[0_0_40px_rgba(0,0,0,0.8)] border border-gray-800 relative group">
                      <img src={absoluteMovieImg} alt={details.title} className="w-full h-auto object-cover" />
                          ...
                          return castList.map((actor, idx) => (
                            <div key={idx} className="flex flex-col items-center gap-2 flex-none w-24 text-center group cursor-pointer">
                              <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-gray-700 shadow-md group-hover:border-netflix-red transition-colors bg-gray-800 flex items-center justify-center">
                                <img src={`${import.meta.env.VITE_MEDIA_BASE || ""}${actor.img}`} alt={actor.name} className="w-full h-full object-cover text-transparent" 
